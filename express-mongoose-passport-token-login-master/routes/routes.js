...
module.exports = function (app, passport, Account) {
  ...
  app.post("/logout", function (req, res) {
    /* Passport exposes a logout() function on req that can be called from
  ...
  app.post("/updatepass", function (req, res) {
    ...
    if (rb.new_password === rb.confirm_password) {
      Account.findByUsername(req.body.email, function (err, user) {
        Account.updatePassword(user, rb.new_password, function () {
          res.redirect("/");
        });
      });
    }
  ...
  app.post("/reset", function (req, res) {
    var rb = req.body;
    if (rb.new_password === rb.confirm_password) {
      Account.authenticate()(req.user.email, rb.password, function (err, user) {
        Account.updatePassword(user, rb.new_password, function () {
          res.redirect("/");
        });
      });
